...
async function sendContactEmail(data) {
  try {
        ...
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; background-color: #000; color: #fff;">
          <div style="background: linear-gradient(135deg, #1a1a1a 0%, #000 100%); padding: 30px; border-radius: 10px; border: 1px solid #d4af37;">
            ...
            <div style="background-color: #1a1a1a; padding: 20px; border-radius: 8px; margin-bottom: 15px; border-left: 4px solid #d4af37;">
              <h3 style="color: #d4af37; margin-top: 0;">Informa\xE7\xF5es do Cliente</h3>
              ...
              <p style="margin: 10px 0;"><strong style="color: #d4af37;">Servi\xE7o de Interesse:</strong> ${data.service}</p>
            </div>
            ...
            <div style="margin-top: 30px; padding-top: 20px; border-top: 1px solid #333; text-align: center;">
              <p style="color: #888; font-size: 12px;">Este email foi enviado atrav\xE9s do formul\xE1rio de contato do site Magnata do Marketing Digital</p>
            </div>
          </div>
        </div>
      `
    ...
    try {
      ...
      console.warn("Aviso: Email n\xE3o p\xF4de ser enviado, mas o contato foi registrado no banco de dados:", sendError);
      return true;
    }
  } catch (error) {
  ...
  contact: router({
    submit: publicProcedure.input(
      z2.object({
        ...
        phone: z2.string().min(1, "Telefone \xE9 obrigat\xF3rio"),
        email: z2.string().email("Email inv\xE1lido"),
        service: z2.string().min(1, "Selecione um servi\xE7o"),
      ...
      try {
        ...
        if (!emailSent) {
          console.warn("Email n\xE3o foi enviado, mas contato foi salvo no banco");
        }
        return {
          success: true,
        ...
        console.error("Erro ao processar contato:", error);
        throw new Error("Erro ao processar sua solicita\xE7\xE3o");
      }
    })
  })
});
